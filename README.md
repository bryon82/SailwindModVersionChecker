...
This works by comparing the version obtained from the BepInEx ChainLoader against the GitHub release tag, GitLab tag, or Thunderstore package version. The repo tag can have any characters in it as long as there is some form of `int`, `int.int`, or `int.int.int` in the tag. When you release a version of your mod, the BepInEx version must match the repo tag version number or the users will get constant update notices.  
